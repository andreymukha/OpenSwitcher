...
- Wayland — обязательная поддерживаемая цель; GNOME Wayland уже подтверждался раньше и запланирован к повторному smoke-тесту перед релизом
...
- Wayland — обязательная поддерживаемая цель, GNOME Wayland является основным Wayland-target
- Другие настольные окружения Linux и Wayland-композиторы считаются best-effort, если явно не указано другое
- KDE не подтверждён как поддерживаемое окружение
- GNOME Wayland уже подтверждался раньше и запланирован к повторному smoke-тесту перед релизом
...
- Первый релиз протестирован на Linux Mint Cinnamon X11. Wayland — обязательная поддерживаемая цель, GNOME Wayland является основным Wayland-target.
- Другие настольные окружения Linux и Wayland-композиторы считаются best-effort, если явно не указано другое.
- KDE не подтверждён как поддерживаемое окружение.
- GNOME Wayland уже подтверждался раньше и запланирован к повторному smoke-тесту перед релизом.
- Исключения для отдельных приложений не входят в первый релиз.
