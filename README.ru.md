...
| --- | --- | --- | --- |
| Linux Mint 22.2 Cinnamon | X11 | Поддерживаемый baseline | Подтверждено проверкой exact DEB в ВМ |
| Ubuntu 24.04 LTS GNOME 46 | Wayland | Поддерживаемая Wayland-цель | Подтверждено проверкой exact DEB в ВМ |
...
- Официальная модель запуска и автозапуска зависит от `systemd --user`.
- Конвертация выделенного текста следует описанной выше clipboard policy: восстановление текста условно, а успешная операция намеренно заменяет прежнее нетекстовое содержимое преобразованным текстом.
