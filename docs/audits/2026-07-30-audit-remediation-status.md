...
- пользовательский smoke одной из локальных сборок `0.1.0-8` на host; smoke
  точного финального артефакта остаётся последним ручным шагом.
...
3. На хосте остаётся только ручная переустановка точного финального артефакта
   и короткий пользовательский smoke; текущий установленный `0.1.0-8` является
   другой повторной сборкой той же Debian-версии.
...
8. ограниченный read-only host acceptance — **пройден для установленной
   версии; exact artifact smoke остаётся ручным шагом**.
