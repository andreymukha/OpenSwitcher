...
build-команда:
  build                 Собрать open-switcher, tray и settings без запуска
...
print_dev_runtime_retirement() {
    cat >&2 <<'EOF'
Прямой dev-runtime удалён: manage.sh больше не запускает и не останавливает процессы по PID.
Для сборки используй:
  ./manage.sh build
Для реальной установки и проверки используй:
  ./manage.sh package deb
Для явного локального managed-пути доступны команды:
  ./manage.sh systemd <команда>
EOF
}
...
case "$namespace" in
    ...
    bootstrap)
        ...
        ;;
    dev|start|stop|restart|status|logs|settings)
        print_dev_runtime_retirement
        exit 1
        ;;
    "")
