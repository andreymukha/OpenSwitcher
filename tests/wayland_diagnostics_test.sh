...
test_wayland_doctor_reports_trusted_gnome_wayland_context() {
    ...
    assert_contains "$output" "Tray acceptance:"
    assert_contains "$output" "Tray visibility: required for supported environment acceptance"
    assert_contains "$output" "Tray absence: does not by itself prove daemon/D-Bus/settings failure"
    assert_contains "$output" "Tray missing checks: daemon status, D-Bus response, settings window, tray service status, tray service logs"
